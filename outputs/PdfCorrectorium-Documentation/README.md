...
初版要求ベースラインに、dev.129までの実装差分・検証結果と残件を追記した状態。ライブラリの正確なバージョン、PDF適合範囲、未完了のUI・復旧・OCRプロバイダー境界は、実装とADRの更新に合わせて確定する。要求を現行の不具合に合わせて緩和したものではない。

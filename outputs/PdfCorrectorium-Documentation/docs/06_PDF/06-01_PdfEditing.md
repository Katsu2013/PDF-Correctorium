...
矩形、文字境界、多角形、フリーハンドは、いずれもPDFポイントの閉じた輪郭へ正規化する。非矩形は外接矩形内の走査線と輪郭辺の交点を並べ、偶奇規則で内部区間だけを保存色で塗る。1ポイントの安全余白は走査位置と区間端へ適用する。矩形は従来の一括塗りを使う。

不可視OCRの保持可否と出力後の抽出文字検証は`PdfRedactionGeometry`の同じ輪郭交差判定を使う。外接矩形の隅にあるだけで実輪郭外の文字を消さず、実輪郭と接する文字は安全余白込みで除去する。自己交差し得るフリーハンド輪郭は偶奇規則で扱う。

...
## dev.173のリボン表示状態とコンテキスト境界

- リボン／クラシックの正本は`ApplicationSettings.UseRibbonUi`とし、ViewModelの`IsRibbonUiMode`／`IsClassicUiMode`は同じ値から派生する。画面だけの独立フラグを持たず、切り替え時に既存の原子的な設定保存と同時起動マージを通す。
- 設定形式17で項目を追加し、旧JSONで未指定ならリボンを既定とする。UIモードはPDF内容、プロジェクト形式、Undo履歴、ワークスペースプリセットを変更しない。
- `EditorInteractionMode`をリボンのコンテキスト表示にも再利用し、OCR編集／読み順／校正・確認／墨消しの可視操作と実コマンドの可否を同じ状態から導く。架空コマンドやモード別の重複状態を作らない。
- QATはUIモードの外側に置き、保存・出力・Undo・RedoとUI切り替えを共通化する。F6巡回だけが可視UIに応じてリボンまたはクラシックツールバーを先頭ペインとして選ぶ。

## dev.163の輪郭墨消し境界

- Coreの`PdfRedactionGeometry`へ、矩形互換輪郭、外接矩形、点包含、矩形交差を集約する。ViewModel、出力、出力後検証が別々の形状判定を持たない。
- WPFは入力軌跡と保存輪郭の表示を担当し、PDFポイント列だけを永続化する。フリーハンド点列はViewModelで間引き、描画中のマウスイベント数をそのままデータ量へしない。
- 出力は輪郭の外接範囲内だけを走査するスキャンライン塗りを用いる。ページ全画素と全頂点の直積処理を避け、矩形データは従来の高速な矩形塗りを維持する。
- PDFプレビュー結果はOCR編集領域とは別に、元PDFの可視／不可視文字境界を返す。文字選択はこの読取専用情報から墨消しを作り、元PDFオブジェクトを編集しない。

...
dev.129ではページ追加・削除・並べ替え・90度回転をCtrl+Z／Ctrl+Yで取り消し・再実行できる。ページ操作はOCR編集と同じ時系列の履歴へ入り、ページ操作を戻した後は、その前に行ったOCR文字・配置編集も続けて戻せる。復元対象は作業PDF、OCRページ、しおり、表示ページ、ページ複数選択であり、元PDFは変更しない。ページ削除の確認画面にもUndo可能であることを表示する。

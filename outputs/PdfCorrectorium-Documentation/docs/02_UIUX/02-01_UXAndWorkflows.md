...
## dev.174のファイルBackstage

1. リボン左端の【ファイル】またはAlt+FでBackstageを開く。QATは残し、リボン内容、左右ペイン、PDFプレビュー、ステータスバーはファイル操作画面へ置き換える。
2. 左側でPDF／プロジェクト／OCRデータの読込、上書き・別名保存、PDF出力、文書プロパティ、設定、バージョン情報、終了を選ぶ。右側には最近開いたファイルをファイル名・保存場所・フルパスで表示する。
3. ファイル操作を選ぶとBackstageを閉じてから処理する。戻るまたはEscでは処理を行わず、直前のリボンタブへ戻る。
4. Backstageを開閉してもPDFのページ、倍率、選択、編集モードを変更しない。クラシックUIへ切り替えた場合はBackstageを閉じ、従来のファイルメニューを使う。
5. F6／Shift+F6は戻る、ファイル操作、最近開いたファイルを巡回する。固定文言、ツールチップ、Automation名は日本語／英語を切り替える。

## dev.173のリボンUIとUIモード切り替えフロー

1. 利用者は最上部QATの右端、またはクラシック表示時のメニュー／ツールバーからリボン／クラシックを切り替える。選択は直ちに保存され、次回起動も同じUIから始まる。
2. dev.174以降は【ファイル】で文書を開き、【ホーム】でOCR編集／読み順編集／校正・確認／墨消しを選ぶ。短縮ラベルではなく、動作が分かる日本語ラベルと専用アイコンを使う。
3. 【編集・校正】へ移ると、現在モードに必要な操作だけが現れる。OCR編集では編集単位と領域・文字送り、読み順では順序移動と再計算、校正では対象の絞り込み・移動・確認、墨消しでは指定方法と範囲の追加／削除／解除を使う。
4. 【表示・検証】ではパネル、単一／見開き、ページ切り替え／連続スクロールに加え、表紙単独表示、左右綴じ、OCR品質、文書情報、検証、復旧を同じ場所から選ぶ。見開き詳細は見開き時だけ有効になる。
5. QATはAlt+1～5、タブはAlt+H/E/Vで操作できる。`F6` / `Shift+F6` は表示中のリボンまたはクラシックツールバーから各ペインを巡回する。Tab移動時は選択状態とは別に明瞭なフォーカス枠を表示する。

## dev.163のPDF文字・多角形・フリーハンド墨消し

1. 墨消しモードの「入力方法」から、矩形、PDF文字を選択、多角形、フリーハンドを選ぶ。
2. PDF文字は対象文字を囲むようにドラッグする。元PDFに文字境界が存在する可視文字と不可視文字だけが個別の墨消しとなり、画像だけの文字は対象にならない。
3. 多角形は頂点を順にクリックし、ダブルクリックまたはEnterで確定する。フリーハンドは押したまま輪郭を描き、離すと閉じた範囲として確定する。確定前のEscは描画だけを中止する。
4. 作成後は従来どおり外接枠をドラッグして移動し、8方向ハンドルで輪郭全体を比例変形する。色変更、Delete、Undo/Redo、半透明／不透明表示も全形状に共通する。

PDF文字の文字境界、プレビュー座標、PDFポイントの変換はページ描画時の同じ変換を使う。入力方法の変更で未確定軌跡を残さず、確定後だけ永続編集とする。
...
- 一般的なPDF閲覧ソフトに近い既定値を採用するが、特定の商標・完全互換をうたわない。

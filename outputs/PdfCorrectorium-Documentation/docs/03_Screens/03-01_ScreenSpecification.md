...
## dev.174のファイルBackstage（2026-09-23）

- リボンの最左端に【ファイル】(Alt+F)を置く。通常の98 DIPコマンド領域は持たず、選択時にQATより下のクライアント領域をBackstageで置き換える。
- 上段に戻るボタン、ファイル見出し、Esc案内を置く。左列は「開く」「保存と出力」「管理」、右列は「最近開いたファイル」とする。
- 左列にはPDF／プロジェクト／OCRデータ、上書き保存、別名保存、PDF出力、文書プロパティ、設定、バージョン情報、終了を置く。ホームには同じファイル操作を置かない。
- 右列はファイル名と保存場所を主表示し、フルパスを副表示とツールチップへ出す。履歴がない場合は空状態を明示する。
- 戻るまたはEscで直前の非ファイルタブへ復帰する。Backstage表示中のF6／Shift+F6は、戻る、左操作列、最近使ったファイルを巡回する。
- 別ウィンドウは作らず、背後の作業画面を保持する。クラシックUIへの切替時は閉じる。

## dev.173のリボンUIとUI切り替え（2026-09-23）

詳細仕様はリポジトリ直下の [リボンUI・UI切り替え仕様 (RIBBON-UI.md)](../../../RIBBON-UI.md) を参照。

- 最上部に高さ36 DIPのQATを置く。上書き保存(Ctrl+S/Alt+1)、PDF出力(Alt+2)、Undo(Ctrl+Z/Alt+3)、Redo(Ctrl+Y/Alt+4)、UIモード切替(Alt+5)を常設し、各アイコンボタンに専用ベクター、ツールチップ、Automation名を与える。
- メイン操作部は3タブ構成とし、コマンド領域98 DIP、大ボタン78 DIP、小ボタン28 DIPを基準にする。ホバー、選択、無効、キーボードフォーカスは別の外観で示す。
  - **【ホーム】(Alt+H)**: 文書、主要編集モード（OCR編集/読み順/校正・確認/墨消し）、表示・移動、検索・校正。
  - **【編集・校正】(Alt+E)**: 現在モードに連動し、OCR編集では編集単位・領域・配置・文字送り、読み順では前後移動と再計算、校正では絞り込み・対象移動・確認、墨消しでは指定方法と範囲操作だけを表示する。
  - **【表示・検証】(Alt+V)**: パネル表示、ページ配置、表紙単独表示、綴じ方向、OCR品質、文書情報、プロジェクト検証、バックアップ復旧。
- 左ペイン（ページ一覧／しおり）上のツールバーはコンテキスト操作を重視してそのまま維持する。
- 従来のメニューバー＋ツールバー（クラシックUI）とリボンUIを相互に切り替える。選択は設定形式17の`UseRibbonUi`へ直ちに保存し、次回起動時に復元する。形式16以前で項目がない場合はリボン表示へ移行する。
- どちらのUIでもF6/Shift+F6によるペイン巡回を維持する。リボンタブのフォーカス表示を消さず、テンプレート上でも2 DIPのフォーカス枠を示す。
...
- ダブルクリックやホイール修飾操作
- パネルの既定配置と最小幅
- 一般的なPDF標準操作に準拠したショートカットの正確な対応表

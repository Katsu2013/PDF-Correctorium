...
dev.123では別文書を開く際にも保存確認を行う。読込失敗時には元の文書と編集を保持する。ページ構造操作のUndoなどの残件は[実装状況](../../../../IMPLEMENTATION_STATUS.md#known-implementation-defects)を参照する。
...
現行実装では「表示」「編集」「ショートカット」「保存・復旧」「保存先」の5タブを提供する。表示ではUI言語、ツールバー・パネル・サムネイル、OCRオーバーレイ、文字セル、文字幅調整ハンドル、領域サイズ変更ハンドルを設定できる。編集ではUndo履歴数と画像支援文字幅推定、ショートカットでは主要操作のキー割当、保存・復旧では自動保存間隔とバックアップ世代数を設定できる。保存先では現在の動作モードと設定ファイルの場所を読み取り専用で表示する。dev.123では操作停止約30秒後にも保存し、未保存プロジェクトの復旧先についても説明する。OCR、プロジェクト修復、プライバシー、ログ、プラグイン、詳細の専用カテゴリは未実装である。

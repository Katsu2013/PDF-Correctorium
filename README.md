...
文字選択中は、通常の大きな選択矩形ではなく、選んだ行に沿うマーカー帯をその場で表示します。帯は前後行や隣文字を隠さないよう、縦余白を文字高の3%（0.5～1.2表示ピクセル）、横余白を0.25～0.75表示ピクセルとしています（※PDF出力時の追加安全余白0.1ポイントは、dev.171で完全幾何学一致のため廃止され、編集画面の座標寸法そのまま出力されます。交差文字の判定安全余白は維持）。確定後は自動的に選択を解除し、文字に結び付いた帯は移動・サイズ変更できません。帯のクリックで削除対象として再選択でき、ページ余白のクリックまたはEscapeで選択を解除できます。矩形・多角形・フリーハンドの任意範囲は、機密情報を残さないため従来どおり対象ページを高精細画像へ変換します。
...
矩形・多角形・フリーハンド等で使用される安全側フォールバックの画質については、墨消しページに画像最適化を先に適用してから再画像化する二重JPEG圧縮を廃止し、元の表示内容から直接300dpi・JPEG品質99で生成します（※文字選択で作った墨消しはdev.167以降画像化フォールバックを行わず、ベクター構造を保持します）。A4相当では2481×3508画素となります。極端に大きなページでは、安定性のため長辺14,000画素・約96メガピクセルを上限とします。このフォールバックを使ったページでは、既存注釈、構造タグ、ベクター情報は画像化されます。
...
dev.159では、大量ページの出力時に不要となったPDFiumのページリソースが蓄積し、隔離ワーカーが2 GiBの安全上限に達する問題を修正しました。最大96ページごと、またはプライベートメモリが768 MiBに達した時点（早い方）で、作業中PDFのチェックポイント作成・圧縮・再オープンを行います。各チェックポイント中は専用の進捗フェーズを表示し、文字送り補正の進捗は成功行・フォールバック行の両方で進行し、繰り返されるフォールバック警告は数万件のメッセージをUIや状態ファイル・ログへ展開せず件数と代表例に集約します。実際の450ページ、14,275変更領域、444画像最適化、1墨消しを含むプロジェクトで、2 GiBのジョブ上限内で約158秒で出力を完了しました。
...
dev.152では、選択文字・OCR領域または任意の矩形を墨消し範囲として指定し、`#RRGGBB`色を選択してプロジェクトへ保存できる基礎機能を導入しました。指定はUndo/Redoでき、元PDFは変更しません。当初の確定出力は対象ページ全体の検索・コピーを失う保守的方式でしたが、dev.160で範囲外OCRを保持し、dev.161で部分交差する行も文字境界単位で保持・除去する方式へ更新しています（さらにdev.167以降は文字選択墨消しの画像化を廃止し、ベクター構造を直接保持する方式へ刷新されています）。元PDFを保持するプロジェクト、自動保存、バックアップは機密情報を含み得るため、配布用の墨消し済みPDFとして扱わないでください。
...
Dev.168 separates the preview marker band from PDF character-removal decisions and output geometry. A character is removed only when its center falls inside the saved band, and the output rectangle is tightened from the native PDF coordinates of the selected characters, preventing glyph overhangs, preview rounding, and marker margins from removing or obscuring neighboring characters. In a real 3-band saved project, 29 characters were removed while preserving adjacent brackets, `CLI`, and particles as visible and extractable.

"Select PDF Text" can select a text line at the pointer position even with horizontal dragging with minimal Y movement (and narrow vertical drags for vertical text). This helper area is used solely to discover characters and is not added to the redaction band. The cursor changes to an I-beam instead of a crosshair.

Redactions created by text selection do not fall back to page images. Marks on original text commands are parsed after save, removing only the character codes of selected characters in `Tj`/`TJ` commands and appending opaque rectangles as native PDF objects. Both hexadecimal and literal string formats are supported, preserving fonts, character widths, positions of out-of-range text, and original images/graphics. Annotations alone do not trigger rasterization; only annotations intersecting the redaction area are removed, keeping other annotations intact. Where safe partial editing of original commands is impossible (e.g. text in forms or rotated text), PDF export is aborted with an explanation rather than falling back to rasterization.

A real 205-page PDF verified that 15 fonts and 8 images/masks on target pages were preserved, adding no new full-page bitmaps, removing only 29 characters across 7 original text commands while keeping 19 out-of-range characters intact. Post-export validation re-verifies 0 extractable characters in protected zones.

During text selection, a marker band is displayed along the line instead of a large marquee box. Text bands use compact padding (vertical padding of 3% / 0.5–1.2 preview px, horizontal padding 0.25–0.75 px; note that the earlier 0.1-point export safety margin was eliminated in dev.171 to guarantee exact geometry between preview and output, while keeping the security margin for character-intersection checks). After confirmation, selection is cleared automatically, and bands bound to text cannot be moved or resized. Clicking a band selects it for deletion, and clicking outside or pressing Escape deselects it. Arbitrary rectangle, polygon, and freehand ranges continue to fall back to high-resolution page rasterization to ensure no confidential data remains.
...
Dev.152 introduced persistent, undoable redaction marks for selected characters, OCR regions, and arbitrary rectangles with `#RRGGBB` colors. Its original conservative export removed search and copy from the whole affected page. Dev.160 retained invisible OCR outside protected ranges, and dev.161 additionally preserves the safe parts of partially intersecting lines at character-boundary granularity (furthermore, dev.167 completely eliminated rasterization fallback for text selections in favor of native vector preservation). Source-bearing projects, autosaves, and backups can still contain the confidential source and must not be distributed as redacted output.
